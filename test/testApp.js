...
describe('/', () => {
  it('should redirect to home page', (done) => {
    request(app).get('/').expect('Location', '/user/home').expect(302, done);
  });
});
...
describe('/user/home', () => {
  ...
  it('should serve home page', (done) => {
    ...
    app.locals.sessions = sessions;
    request(app).get('/user/home').set('Cookie', 'xyz').expect(200, done);
  });
});

describe('login page', () => {
  it('should serve login page when requested for /login.html', (done) => {
    request(app).get('/login.html').expect(200, done);
  });
});
...
describe('verify', () => {
  ...
  it('should verify user when requested for /verify', (done) => {
    request(app).get('/verify').expect(302, done);
  });
});
