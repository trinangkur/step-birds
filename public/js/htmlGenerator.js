...
const createContent = function (content) {
  ...
  return words.reduce((contentHtml, word) => {
    const wordHtml = word[0] === '#' ? `<a href="/hello">${word}</a>` : word;
  ...
  return `<div class="parent-element" id="content-${id}" onClick="openTweet(${id})">
            ...
            <div class="info">
              <div class="user-info">
                ...
                <span></span>
                <span class="time-stamp"> &nbsp; ${moment(
                  timeStamp
                ).fromNow()}</span>
              </div>
              <div class="content" id="content-${id}" onClick="openTweet(${id})">${createContent(
    content
  )}
              </div>
            </div>
...
const createContentHtml = function (tweet, reference) {
  ...
  return `<div class="content">
            <div id="content-${id}" onClick="openTweet(${id})">${createContent(
    content
  )}</div>
            ${parentTweetHtml}
          </div>`;
